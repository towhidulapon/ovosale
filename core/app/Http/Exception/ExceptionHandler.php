...
class ExceptionHandler
{
    ...
    public function exceptionShowApi($e)
    {
        ...
        if ($e instanceof NotFoundHttpException) {
            return jsonResponse('not_found', 'error', [$exceptionMessage], statusCode: 404);
        }

        if ($e instanceof UnauthorizedException) {
            return jsonResponse('restricted', 'error', [$exceptionMessage], statusCode: 403);
        }
        //for authenticated 
        if ($exceptionMessage === 'Unauthenticated.') {
            $notify[] = 'Unauthorized request';
            return jsonResponse('unauthorized', 'error', $notify, statusCode: 401);
        }
        //for validation
        if ($e instanceof ValidationException) {
            ...
            return jsonResponse('validation_error', 'error', $allErrors);
        }

        return jsonResponse('exception', 'error', [$exceptionMessage]);
    }
    ...
    public function exceptionShowForCustomNotFound($e)
    {
        ...
        if (isApiRequest()) {
            ...
            return jsonResponse('not_found', 'error', $notify);
        } else {
