...
<script>
    ...
    const triggerToaster = (status, message, position = "topRight") => {
        iziToast[status]({
    ...
    function notify(status, message, position = "topRight") {
        if (typeof message == 'string') {
            triggerToaster(status, message, position);
        } else {
            $.each(message, (i, val) => triggerToaster(status, val, position));
        }
    }
</script>
