...
<div class="row responsive-row">
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.purchase.list') }}?date={{ $today }}" variant="primary"
            title="Today Purchase" :value="$widget['today_purchase']" icon="las la-calendar" />
    </div>
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.purchase.list') }}?date={{ now()->subDay()->format('Y-m-d') }}"
    ...
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four
            url="{{ route('admin.purchase.list') }}?date={{ now()->subDays(7)->format('Y-m-d') }}to{{ $today }}"
    ...
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four
            url="{{ route('admin.purchase.list') }}?date={{ now()->subDays(30)->format('Y-m-d') }}to{{ $today }}"
            variant="primary" title="Last 30 Days Purchase" :value="$widget['last_30days_month_purchase']" icon="las la-calendar" />
    </div>
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.purchase.list') }}" variant="info" title="All Purchase"
            :value="$widget['all_purchase']" icon="las la-calendar" />
    </div>
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.purchase.list') }}" variant="primary" title="Last Purchase Amount"
