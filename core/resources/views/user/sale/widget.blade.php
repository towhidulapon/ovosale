...
<div class="row responsive-row">
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.sale.list') }}?date={{ $today }}" variant="primary"
            title="Today Sale" :value="$widget['today_sale']" icon="las la-calendar" />
    </div>
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.sale.list') }}?date={{ now()->subDay()->format('Y-m-d') }}"
    ...
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four
            url="{{ route('admin.sale.list') }}?date={{ now()->subDays(7)->format('Y-m-d') }}to{{ $today }}"
    ...
    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four
            url="{{ route('admin.sale.list') }}?date={{ now()->subDays(30)->format('Y-m-d') }}to{{ $today }}"
            variant="primary" title="Last 30 Days Sale" :value="$widget['last_30days_month_sale']" icon="las la-calendar" />
    </div>

    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.sale.list') }}" variant="info" title="All sale"
            :value="$widget['all_sale']" icon="las la-calendar" />
    </div>

    <div class="col-xxl-3 col-sm-6">
        <x-admin.ui.widget.four url="{{ route('admin.sale.list') }}" variant="primary" title="Last Sale Amount"
