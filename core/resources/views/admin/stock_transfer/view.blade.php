...
<div class="row  responsive-row">
    <div class="col-12">
        @include('admin.stock_transfer.invoice')
    </div>
        <div class="col-12">
            <div class="d-flex flex-wrap align-items-center  gap-3">
                <x-permission_check permission="edit stock transfer">
                    <a class="btn btn--primary" href="{{ route('admin.stock.transfer.edit', $transfer->id) }}">
                        <i class="las la-pencil-alt"></i>
                        @lang('Edit Transfer')
                    </a>
                </x-permission_check>
                <x-permission_check permission="view stock transfer">
                    <a class="btn btn--warning" href="{{ route('admin.stock.transfer.pdf', $transfer->id) }}">
                        <i class="las la-file-download "></i>
                        @lang('Download PDF')
                    </a>
                </x-permission_check>
                <x-permission_check permission="view stock transfer">
                    <a class="btn btn--info" href="{{ route('admin.download.attachment', encrypt(getFilePath('stock_transfer_attachment') . '/' . $transfer->attachment)) }}">
                        <i class="las la-file-download "></i>
                ...
                <x-permission_check permission="view stock transfer">
                    <x-back_btn route="{{ route('admin.stock.transfer.list') }}" />
                </x-permission_check>
            </div>
        </div>
    </div>
